...
function initKanban(diagramID) {
    const nodeDataArray = [
        ...
        },
        {
            ...
            "group": "Customer",
            "color": "0",
            "loc": "683 155.52284749830795"
        }
    ];
    const linkDataArray = [];
    kanbanDiagram = gojs(go.Diagram, "diagramDiv-" + diagramID,  // create a Diagram for the DIV HTML element
        {
        ...
        gojs(go.Node, "Horizontal",
            ...
            gojs(go.Shape, "Rectangle", {
                    ...
                    click: function (e, obj) {
                        kanbanDiagram.startTransaction("Update node color");
                        let newColor = parseInt(obj.part.data.color) + 1;
                        if (newColor > noteColors.length - 1) newColor = 0;
                        kanbanDiagram.model.setDataProperty(obj.part.data, "color", newColor);
                        kanbanDiagram.commitTransaction("Update node color");
                    }
                },
    ...
    kanbanDiagram.nodeTemplateMap.add('newbutton',
        gojs(go.Node, "Horizontal",
            {
                selectable: false,
                click: function (e, node) {
                    kanbanDiagram.startTransaction('add node');
                    const newdata = {
                        group: "Problems",
                        loc: "0 50",
                    ...
                    kanbanDiagram.model.addNodeData(newdata);
                    kanbanDiagram.commitTransaction('add node');
                    var node = kanbanDiagram.findNodeForData(newdata);
                    kanbanDiagram.select(node);
                    kanbanDiagram.commandHandler.editTextBlock();
                },
                background: 'white'
            },
            gojs(go.Panel, "Auto",
    ...
    function highlightGroup(grp, show) {
        if (show) {
            const part = kanbanDiagram.toolManager.draggingTool.currentPart;
    ...
    kanbanDiagram.model = new go.GraphLinksModel(nodeDataArray, linkDataArray);
    kanbanDiagram.delayInitialization(relayoutDiagram());
    const pos = kanbanDiagram.model.modelData.position;
    if (pos) {
        kanbanDiagram.initialPosition = go.Point.parse(pos);
    }
    ...
    desktopDiagram.diagram = kanbanDiagram;
