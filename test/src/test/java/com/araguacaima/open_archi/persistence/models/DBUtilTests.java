...
public class DBUtilTests {
    ...
    private static Logger log = LoggerFactory.getLogger(DBUtilTests.class);
    ...
    public void testCreateModel() {
        try {
            ArchitecturalModel modelObj = jsonUtils.fromJSON(model, ArchitecturalModel.class);
            ArchitecturalModel savedModel = TestDB.process(modelObj);
        } catch (IOException | EntityCreationError e) {
            e.printStackTrace();
            fail();
        }
    }
    ...
    static class TestDB {
        ...
        public static <T> T process(T modelObj) {
            return process(modelObj, true);
        }

        public static <T> T process(T entity, boolean throwError) {
            if (entity == null) {
                return null;
            }
                    ...
                    if (value != null) {
                        if (reflectionUtils.isCollectionImplementation(value.getClass())) {
                            ...
                            if (CollectionUtils.isNotEmpty(value1)) {
                                Set newCollection = new TreeSet();
                                value1.forEach(element -> {
                                    Object innerProcess = process(element, throwError);
                                    newCollection.add(innerProcess);
                                });
                                newCollection.clear();
                                value1.addAll(newCollection);
                            }
                        } else if (reflectionUtils.isMapImplementation(value.getClass())) {
                            Map value1 = (Map) value;
                            if (MapUtils.isNotEmpty(value1)) {
                                Map newCollection = new TreeMap();
                                value1.forEach((key, val) -> {
                                    Object innerProcess = process(val, throwError);
                                    newCollection.put(key, innerProcess);
                                });
                                newCollection.clear();
                                value1.putAll(newCollection);
                            }
                        } else {
                            if (reflectionUtils.getFullyQualifiedJavaTypeOrNull(value) == null) {
                                Object innerValue = process(value, throwError);
                                field.set(entity, innerValue);
                            }
                        }
                    }
                }, Utils::filterMethod);
                log.debug("Processing " + entity.getClass().getName());
                T existentObject = random.nextBoolean() ? entity : null;
                return processPersisted(entity, existentObject);
            } else {
                if (throwError) {
                    throw new EntityCreationError("Object must be an instance of " + BaseEntity.class.getName());
                }
            }
            log.debug("Processing " + entity.getClass().getName());
            T existentObject = random.nextBoolean() ? entity : null;
        ...
        private static <T> T processPersisted(T entity, T existentObject) {
            if (existentObject == null) {
                log.debug("Object not persisted. Saving...");
                return entity;
            } else {
                log.debug("Object already persisted. Replacing...");
